...
def test_frozen_scientific_anchors_match_mvp_spec() -> None:
    ...
    assert STD_IOL_EYE.aperture_mm == 6.0
    assert STD_IOL_EYE.wavelength_nm == 546.0
...
def test_quintic_transition_has_value_first_second_derivative_continuity() -> None:
    eps = 1e-5
    f0 = smoothstep_quintic(0.0).item()
    f1 = smoothstep_quintic(1.0).item()
    d0 = (smoothstep_quintic(eps) - smoothstep_quintic(0.0)) / eps
    d1 = (smoothstep_quintic(1.0) - smoothstep_quintic(1.0 - eps)) / eps
    dd0 = (
        smoothstep_quintic(2 * eps)
        - 2 * smoothstep_quintic(eps)
        + smoothstep_quintic(0.0)
    ) / eps**2
    dd1 = (
        smoothstep_quintic(1.0)
        - 2 * smoothstep_quintic(1.0 - eps)
        + smoothstep_quintic(1.0 - 2 * eps)
    ) / eps**2
...
def test_residual_absence_does_not_fail_core_build_but_blocks_carriers() -> None:
    validations = [
        AssetValidation("LB", NumericalStatus.PASS),
        AssetValidation("A0", NumericalStatus.PASS),
    ]
    ...
    ready = summarize_asset_validations(
        validations, available_residual_platforms=["WFS", "RAD", "HOA"]
    )
