"""Run TASK-014 vertex-corrected postoperative cornea extension.

This task is independent from the frozen TASK-011 Run72. It builds A0V12/B0V12/C0V12,
solves 18 new P/Q carriers, validates frozen residual power coverage, measures 36 paired-MONO
angular references, runs 72 production configs, and archives every final .zmx model.
"""
...
from whole_eye_mvp.task013_native_reference import residual_provenance_from_existing_locks
from whole_eye_mvp.task014_cornea_zos import build_task014_cornea_assets
from whole_eye_mvp.task014_extension import (
    TASK014_CORNEA_IDS,
    TASK014_EXPECTED_CONFIG_COUNT,
    TASK014_EXPECTED_PAIR_COUNT,
    TASK014_EXPECTED_THROUGH_FOCUS_ROWS,
...
from whole_eye_mvp.task014_zos import (
    Task014PairScaleAnalysisBackend,
    measure_task014_pair_mono_reference,
    solve_task014_platform_carrier,
)
...
REPO_EVIDENCE_DIR = REPOSITORY_ROOT / "docs/evidence/task014"
...
def _git_head() -> str:
    ...
    if status:
        raise SystemExit("TASK-014 requires a clean tracked checkout")
    head = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=REPOSITORY_ROOT,
        check=True,
    ...
    if len(head) != 40:
        raise SystemExit("TASK-014 could not resolve canonical Git HEAD")
    return head
...
def _pair_results(results: list[ConfigResult]):
    ...
    for pair_key in sorted(by_pair):
        ...
        if mono is None or edof is None or len(states) != 2:
            raise SystemExit(f"TASK-014 matched pair incomplete: {pair_key}")
        deltas.append(matched_pair_delta(mono, edof))
    if len(deltas) != TASK014_EXPECTED_PAIR_COUNT:
        raise SystemExit("TASK-014 did not reconstruct exactly 36 matched pairs")
    return tuple(deltas)


def _set_hash(records: dict[str, dict[str, object]]) -> str:
...
def _write_repo_evidence(
    *,
    ...
    config_csv = REPO_EVIDENCE_DIR / "TASK_014_CONFIG_RESULTS.csv"
    tf_csv = REPO_EVIDENCE_DIR / "TASK_014_THROUGH_FOCUS.csv"
    delta_csv = REPO_EVIDENCE_DIR / "TASK_014_PAIRED_DELTAS.csv"
    index_csv = REPO_EVIDENCE_DIR / "TASK_014_MODEL_INDEX.csv"
    evidence_json = REPO_EVIDENCE_DIR / "TASK_014_EVIDENCE.json"
    ...
    evidence = {
        ...
        if key not in {"completed_result_json_paths", "failure_details", "carrier_evidence"}
    }
    evidence.update(
        {
            "evidence_only": True,
...
def main() -> None:
    ...
    if not isinstance(manifest_artifacts, dict) or not isinstance(residual_hashes, dict):
    ...
    if len(frozen_bundle.physical_carriers) != 18 or len(frozen_bundle.nominal_configs) != 72:
        raise SystemExit("TASK-014 requires unchanged frozen TASK-008 18/72 source")
    residual_provenance = residual_provenance_from_existing_locks(frozen_bundle.physical_carriers)
    ...
    lb_base = next(
        item for item in base_asset_prescriptions(baseline) if item.base_spec.base_id == CORNEA_LOCK_BASE_ID
    )
    lb_base_path = store.resolve(lb_base.relative_path)
    if not lb_base_path.is_file():
        raise SystemExit(f"locked LB base asset missing: {lb_base_path}")

    run_id = f"task014-{uuid.uuid4().hex}"
    model_root = project_dir / "models" / "task014_vertex_corrected"
    cornea_dir = model_root / "corneas"
    carrier_dir = model_root / "carriers"
    run_model_dir = model_root / "runs" / run_id
    p0_archive_dir = run_model_dir / "p0"
    pair_reference_dir = run_model_dir / "pair_references"
    config_archive_dir = run_model_dir / "configs"
    work_dir = project_dir / "diagnostics" / "task014" / run_id / "carrier_work"
    result_root = project_dir / "results" / "task014_vertex_corrected" / run_id
    report_dir = project_dir / "results" / "task014_vertex_corrected" / "reports"
    report_path = report_dir / f"{run_id}.json"
    model_index_path = run_model_dir / "MODEL_INDEX.csv"

    for name in ("REFERENCE_CORNEA.zmx", "DISTANCE_CORNEA_VERTEX12.zmx"):
        _guard(cornea_dir / name, args.overwrite_models)
    for cornea_id in TASK014_CORNEA_IDS:
        _guard(cornea_dir / f"CORNEA_{cornea_id}.zmx", args.overwrite_models)
        for base in (BaseId.LB_AL2395, BaseId.ATC_M3_AL24477):
            for platform in (PlatformId.WFS, PlatformId.RAD, PlatformId.HOA):
                _guard(carrier_dir / f"CAR_{base}_{cornea_id}_{platform}.zmx", args.overwrite_models)
    ...
    with open_zos_session(args.install_dir) as session:
        ...
        cornea_build = build_task014_cornea_assets(session, baseline, lb_base_path, cornea_dir)
        cornea_paths = {key: Path(value) for key, value in cornea_build.cornea_paths.items()}
        for role, path, cornea_id in (
            ("reference_cornea", Path(cornea_build.reference_path), "REFERENCE"),
            ("vertex_corrected_distance_cornea", Path(cornea_build.distance_path), "DISTANCE_V12"),
            *(("vertex_corrected_postop_cornea", path, cid) for cid, path in cornea_paths.items()),
        ):
            model_index.append(
                ModelIndexRecord(
                    model_role=role,
        ...
        p0_map: dict[tuple[str, str], tuple[object, Path]] = {}
        for base in (BaseId.LB_AL2395, BaseId.ATC_M3_AL24477):
            base_id = str(base)
            for cornea_id in TASK014_CORNEA_IDS:
                p0_work = work_dir / f"P0_{base_id}_{cornea_id}.zmx"
                p0 = solve_actual_eye_carrier_power(
                    session,
                    baseline,
                    base_id,
                    cornea_paths[cornea_id],
                    p0_work,
                )
                p0_archive = p0_archive_dir / f"P0_{base_id}_{cornea_id}.zmx"
                p0_sha = archive_model(p0_work, p0_archive)
                model_index.append(
                    ModelIndexRecord(
                        model_role="q0_start_carrier",
                        run_id=run_id,
                        config_id="",
                        pair_key="",
                        carrier_id="",
                        ...
                        optic_state="",
                        pupil_mm="",
                        relative_path=project_relative_path(project_dir, p0_archive),
                        sha256=p0_sha,
                    )
                )
                p0_map[(base_id, cornea_id)] = (p0, p0_archive)
        ...
        for base in (BaseId.LB_AL2395, BaseId.ATC_M3_AL24477):
            ...
            for cornea_id in TASK014_CORNEA_IDS:
                ...
                for platform in (PlatformId.WFS, PlatformId.RAD, PlatformId.HOA):
                    ...
                    carrier_id = f"CAR_{base_id}_{cornea_id}_{platform_id}"
                    destination = carrier_dir / f"{carrier_id}.zmx"
                    carrier, evidence = solve_task014_platform_carrier(
                        session,
                        standard_eye_path,
                        base_id=base_id,
                        cornea_id=cornea_id,
                    ...
                    corrected_carriers.append(carrier)
        ...
        envelope_checks = require_task014_residual_power_envelopes(
        ...
        bundle = build_task014_manifest(locks)
        carrier_hashes = {
        ...
        if len(mono_by_pair) != TASK014_EXPECTED_PAIR_COUNT:
            raise SystemExit("TASK-014 did not resolve exactly 36 MONO pair references")
        ...
        for pair_key in sorted(mono_by_pair):
            ...
            reference = measure_task014_pair_mono_reference(
                session,
                project_dir,
                carrier_dir,
                carrier_hashes,
        ...
        backend = Task014PairScaleAnalysisBackend(
            session,
            project_dir,
            carrier_dir,
            carrier_hashes,
    ...
    deltas = _pair_results(results) if completed == TASK014_EXPECTED_CONFIG_COUNT else ()
    archive_complete = (
        completed == TASK014_EXPECTED_CONFIG_COUNT
        and not failures
        and through_focus_count == TASK014_EXPECTED_THROUGH_FOCUS_ROWS
        and len(deltas) == TASK014_EXPECTED_PAIR_COUNT
    )
    if archive_complete:
    ...
    report = {
        ...
        "rx_contract": asdict(TASK014_RX_CONTRACT),
        "rx_contract_snapshot": task014_prescription_snapshot(baseline),
        "rx_contract_sha256": contract_sha,
        "corrected_cornea_ids": TASK014_CORNEA_IDS,
        "cornea_measurements": cornea_build.measurements,
        "native_manifest_hash": bundle.manifest_hash,
        "native_lock_set_hash": bundle.lock_set_hash,
        "physical_carriers": len(bundle.physical_carriers),
        "nominal_configs": len(bundle.nominal_configs),
        ...
        "pair_reference_set_sha256": _set_hash(pair_reference_records),
        ...
        "task014_complete": archive_complete,
    ...
    if args.write_repo_evidence:
        if not archive_complete:
            raise SystemExit("refusing to write repo evidence for incomplete TASK-014 run")
