"""Run the consolidated TASK-009 grid-based diffraction-MTF validation batch.

Production acquisition uses temporary MFE MTFA operands with ``Grid=1``. This is
the OpticStudio grid-based diffraction-MTF algorithm documented as the alternate
algorithm used by the MTF analysis feature, without instantiating the unavailable
``AS_FftMtf`` Python.NET analysis-settings type.

The batch performs:
1. sampling convergence/repeatability on three frozen EDOF manifest rows;
2. real six-config production-contract integration through ``run_analysis_batch``;
3. MTFA versus (MTFT+MTFS)/2 fixed-frequency consistency diagnostics.

No TASK-005/006/007/008 formal asset is modified and Run72 is not started.
"""
...
from whole_eye_mvp.analysis_zos import (
    TASK009_MTF_ACQUISITION,
...
OUTPUT_REL = Path("diagnostics/task009/mtfa_grid1_representative")
LOCAL_REPORT = "TASK_009_MTFA_GRID1_REPRESENTATIVE.json"
REPO_EVIDENCE_DIR = REPOSITORY_ROOT / "docs/evidence/task009"
REPO_EVIDENCE_NAME = "TASK_009_MTFA_GRID1_REPRESENTATIVE_EVIDENCE.json"
REPO_TF_CSV = "TASK_009_MTFA_GRID1_REPRESENTATIVE_THROUGH_FOCUS.csv"
...
def main() -> None:
    ...
    with open_zos_session(args.install_dir) as session:
        ...
        for _mono, edof in pairs:
            ...
            for sampling in settings.fft_mtf_convergence_samplings:
                backend = ZosMtfaGridAnalysisBackend(
                    session,
                    project_dir,
                    carrier_hashes,
                    residual_hashes,
        ...
        integration_backend = ZosMtfaGridAnalysisBackend(
            session,
            project_dir,
            carrier_hashes,
            residual_hashes,
        ...
        environment = RunEnvironment(
            f"{__version__}+git.{code_commit[:12]}",
            opticstudio_version,
            CURRENT_SCIENTIFIC_BASELINE_ID,
    ...
    report: dict[str, object] = {
        "schema_version": 2,
        "phase": "TASK-009-MTFA-GRID1-REPRESENTATIVE",
        "evidence_only": True,
        ...
        "mtf_acquisition_contract": asdict(TASK009_MTF_ACQUISITION),
        "mtf_acquisition_contract_hash": TASK009_MTF_ACQUISITION.contract_hash,
        ...
        "next_gate": "Web review before formal sampling lock / Run72",
    }
